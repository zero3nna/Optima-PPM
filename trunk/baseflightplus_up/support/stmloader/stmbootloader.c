...
void stmLoader(serialStruct_t *s, FILE *fp, unsigned char overrideParity, unsigned char resetToBoot) {
	char c;
	...
	top:
	if(resetToBoot) {
		printf("Sending R to place Baseflight in bootloader\n");
		c = 'R';
		serialWrite(s, &c, 1);
