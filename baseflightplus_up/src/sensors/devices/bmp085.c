...
static int32_t baroAltitude = 0;
static int32_t baroPressure = 0;
static int16_t baroTemperature = 0;


// About 30 Hz between altitude updates.
void bmp085Update(void)
{
    static uint8_t baroState;
    static uint16_t baroUT;
    static uint32_t baroUP;

    switch (baroState) {
        case 0:
            bmp085_start_ut();
            baroState++;
            singleEvent(bmp085Update, 4600);
            break;
        case 1:
            baroUT = bmp085_get_ut();
            baroState++;
            singleEvent(bmp085Update, 500);
            break;
        case 2:
            bmp085_start_up();
            baroState++;
            singleEvent(bmp085Update, 26000);
            break;
        case 3:
            baroUP = bmp085_get_up();
            baroTemperature = bmp085_get_temperature(baroUT);
            baroPressure = bmp085_get_pressure(baroUP);
            baroAltitude = (1.0f - pow(baroPressure / 101325.0f, 0.190295f)) * 4433000.0f; // centimeter
        default:
            baroState = 0;
            singleEvent(bmp085Update, 5000);
            break;
    }
}
...
{
    *temperature = baroTemperature; // 0.1 c
    *pressure = baroPressure; // pa
    *altitude = baroAltitude; // cm
}
...
bool bmp085Init(void)
{
    ...
    if (p_bmp085->chip_id == BMP085_CHIP_ID) {            /* get bitslice */
        ...
        bmp085InitDone = true;
        // Start the baro update state machine
        singleEvent(bmp085Update, 100000);
        return true;
    }
