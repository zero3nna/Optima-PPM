...
void mixTable(void)
{
    ...
    switch (cfg.mixerConfiguration) {
        ...
        case MULTITYPE_GIMBAL:
            gimbal[0] = cfg.gimbalPitchServoMid + cfg.gimbalPitchServoGain * stateData.pitch * 128 + command[PITCH];
            gimbal[1] = cfg.gimbalRollServoMid + cfg.gimbalRollServoGain * stateData.roll * 128 + command[ROLL];
            break;
        ...
        case MULTITYPE_FLYING_WING:
            ...
            if (mode.PASSTHRU_MODE) {
                // do not use sensors for correction, simple 2 channel mixing
                servo[0]  = cfg.pitchDirectionLeft * (rcData[PITCH] - cfg.midCommand) + cfg.rollDirectionLeft * (rcData[ROLL] - cfg.midCommand);
                servo[1]  = cfg.pitchDirectionRight * (rcData[PITCH] - cfg.midCommand) + cfg.rollDirectionRight * (rcData[ROLL] - cfg.midCommand);
            } else {
                // use sensors to correct (gyro only or gyro + acc)
                servo[0]  = cfg.pitchDirectionLeft * axisPID[PITCH] + cfg.rollDirectionLeft * axisPID[ROLL];
                servo[1]  = cfg.pitchDirectionRight * axisPID[PITCH] + cfg.rollDirectionRight * axisPID[ROLL];
            }
            servo[0] = constrain(servo[0] + cfg.wingLeftMid, cfg.wingLeftMin, cfg.wingLeftMax);
            servo[1] = constrain(servo[1] + cfg.wingRightMid, cfg.wingRightMin, cfg.wingRightMax);
            break;
    }
    ...
    if (featureGet(FEATURE_SERVO_TILT)) {
        ...
        gimbal[0] = cfg.gimbalPitchServoMid + aux[0];
        gimbal[1] = cfg.gimbalRollServoMid + aux[1];

        if (auxOptions[OPT_CAMSTAB]) {
            gimbal[0] += cfg.gimbalPitchServoGain * stateData.pitch * 128;
            gimbal[1] += cfg.gimbalRollServoGain * stateData.roll * 128;
        }
    }
    
    if(featureGet(FEATURE_SERVO_TILT) || cfg.mixerConfiguration == MULTITYPE_GIMBAL) {
        servo[0] = filterSmooth(gimbal[0], servo[0], cfg.gimbalSmoothFactor);
        servo[1] = filterSmooth(gimbal[1], servo[1], cfg.gimbalSmoothFactor);
